...
int beat_sequence_length = 4;   // How many beats until the sequence repeats.
int beat_index = 0;
...
void process_beat_track(){
  ...
  if (beat_progress >= sixteenth_interval){
    ...
    sixteenth_count++;
    Serial.println(sixteenth_count);
...
void play_note(uint8_t channel, uint8_t note){
  ...
  set_attenuation(channel, 0b00000010);
